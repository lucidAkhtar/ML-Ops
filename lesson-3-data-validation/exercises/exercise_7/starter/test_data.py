...
# A decorator to pass the input data/variables to the tests.
...
def data():
    ...
    local_path = run.use_artifact(
        "exercise_5/pre_processed_data:latest").file()
...
def test_column_ranges(data):
    ...
    for col_name, (minimum, maximum) in ranges.items():
        ...
        assert data[col_name].dropna().between(minimum,maximum).all(),(
            "Column {} values are not between {} and {}".format(col_name,minimum,maximum)
        )
